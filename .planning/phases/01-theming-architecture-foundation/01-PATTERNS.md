...
- The **CDN `<script>`** is at line 7, **before** the inline config at lines 9–46. RESEARCH §"Recommended Project Structure" calls this out as ordering that *works today* (the CDN script reads `window.tailwind.config` at execute-time, after the inline config has set it) but is **load-bearing-fragile** for Phase 1. The planner must move the CDN script to land **after** the new token `<style>`. D-10 specifies this ordering explicitly.
